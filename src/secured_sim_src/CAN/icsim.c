 ...
 *
 * (c) 2014 Open Garages - Craig Smith <craig@theialabs.com>   [original ICSim]
 * Forked version (new GUI, UDS, challenges): phil-eqtech, Barbhack CH-Workshop.
 *
 * Security layer for this secured build
 *   Copyright (c) 2026 Xpert <ahmad.pc.saad@gmail.com> - IMT Atlantique, IoV Security Lab
 *   - SecOC verify-or-drop gate on protected signal frames (see secoc.c/.h)
 *   - UDS hardening: CMAC SecurityAccess, anti-brute-force lockout,
 *     RoutineControl rate limiting, authorization gating of the secret routine
 */
...
uint8_t diagSeed[SEC_SEED_LEN];        // random seed sent to the tester
uint8_t expectedKey[SEC_SEED_LEN];     // CMAC-derived key we expect back
char seedGenerated = 0;
char secretSessionFound = 0;
// SecurityAccess anti-brute-force state
int  secLockUntil  = 0;                // currentTime until which 0x27 is blocked
int  secFailCount  = 0;                // consecutive invalid-key attempts
// RoutineControl rate-limit state
int  routineLastTime = 0;              // currentTime of the last 0x31 request
...
void sendIsoTpData() {
  ...
  for (; i < 8 && isoTpRemainingBytes > 0; i++) {
...
/* ===========================================================================
 * SECURITY BLOCK 2 - UDS diagnostic hardening
 * ===========================================================================
 * Plain-English summary:
 *   "Diagnostics" (UDS, the protocol a garage tool uses to talk to the car)
 *   is a second way in. The unsecured simulator let an attacker walk right
 *   through it: open a privileged session for free, guess the security key in
 *   a handful of tries, and discover a hidden command by brute force.
 *
 *   This function is the diagnostic message handler, and it contains four
 *   small sub-blocks that close those holes (all using standard, real-world
 *   ISO 14229 UDS mechanisms):
 *       BLOCK 2A - strong SecurityAccess key (a real crypto challenge that
 *                  cannot be guessed instead of the old, predictable one)
 *       BLOCK 2B - anti-brute-force throttling and lockout (each wrong key
 *                  costs a forced wait; too many in a row triggers a lockout)
 *       BLOCK 2C - authorization gate on the hidden routine (you must pass
 *                  SecurityAccess first; just opening the session is not enough)
 *       BLOCK 2D - rate limiting on routine requests (defeats the scanner that
 *                  tries every possible routine number as fast as it can)
 *
 *   Each sub-block is labelled inline below. See SECURITY_LAYER.md for details.
 * ------------------------------------------------------------------------- */
void analyseDiagRequest(struct canfd_frame *frame, int maxdlen) {
  ...
  if (frame->data[0] < 0x09 && frame->data[0] > 0) {
    ...
    switch (frame->data[1]) {
      ...
      case UDS_SID_TESTER_PRESENT:
        ...
        case UDS_SID_ROUTINE_CONTROL:
          // --- SECURITY BLOCK 2D: RoutineControl rate limiting ---------------
          // Why: a "routine" is identified by a 3-byte number. An attacker can
          // try every number to find the hidden one. We make that slow.
          // How: reject any routine request that arrives less than
          // ROUTINE_MIN_INTERVAL_MS after the previous one (busyRepeatRequest,
          // 0x21). Because a flood keeps resetting the clock, it stays blocked,
          // turning a "minutes" brute-force into a "weeks" one. A real garage
          // tool that sends the odd routine is unaffected.
          {
            int gap = currentTime - routineLastTime;
            routineLastTime = currentTime;
            if (gap < ROUTINE_MIN_INTERVAL_MS) {
              sendFrameError(UDS_SID_ROUTINE_CONTROL, UDS_ERROR_BUSY_REPEAT);
              return;
            }
          }
          if (frame->data[0] == 4) {
            if (diagSession == 2) {
              if (frame->data[2] == 0x41) {
                if (frame->data[3] == 0x10 || frame->data[3] == 0x22) {
                  // --- SECURITY BLOCK 2C: authorization gate ----------------
                  // Why: the hidden routine (0x22) used to run as long as you
                  // were in the diagnostic session, which anyone could enter.
                  // How: require that SecurityAccess has actually SUCCEEDED in
                  // this session (secretSessionFound == 1). If not, refuse with
                  // securityAccessDenied (0x33). Now the only way to reach the
                  // secret routine is through BLOCK 2A/2B, which cannot be
                  // brute-forced - so just opening the session is not enough.
                  if (frame->data[3] == 0x22 && secretSessionFound != 1) {
                    sendFrameError(UDS_SID_ROUTINE_CONTROL, UDS_ERROR_SECURITY_DENIED);
                    return;
                  }
        ...
        case UDS_SID_SECURITY_ACCESS:
          ...
          // --- SECURITY BLOCK 2B: anti-brute-force throttling (gate) --------
          // Why: even with a strong key, an attacker could script thousands of
          // guesses per second. We make each attempt expensive in time.
          // How: after a wrong key (see the matching part of this block, lower
          // down) we set secLockUntil. Until that moment passes, every
          // SecurityAccess request is refused with requiredTimeDelayNotExpired
          // (0x37). After too many wrong keys the wait becomes a long lockout.
          // Important: this counter/lockout is NOT reset by changing session or
          // resetting the ECU, so an attacker cannot wipe it - just like a real
          // ECU that remembers failures in non-volatile memory.
          if (currentTime < secLockUntil) {
            sendFrameError(UDS_SID_SECURITY_ACCESS, UDS_ERROR_TIME_DELAY);
            return;
          }
          ...
          switch (frame->data[2]) {
            case 0x01: {
              ...
              // --- SECURITY BLOCK 2A: strong SecurityAccess key -------------
              // Why: the old scheme answered a 2-byte seed with
              // "seed XOR two-VIN-bytes" - a simple, public rule an attacker
              // cracked in a few dozen tries.
              // How: we send a fresh 4-byte RANDOM seed and the only correct
              // answer is AES-128-CMAC(secret_key, seed) (computed by
              // secoc_sec_response, reusing the SecOC key). Because the answer
              // depends on a secret key AND a random seed, an attacker on the
              // bus cannot compute or predict it - there is no shortcut to
              // discover, unlike the old XOR rule. The reply is checked in
              // constant time further down (still part of BLOCK 2A/2B).
              int i;
              for (i = 0; i < SEC_SEED_LEN; i++)
                diagSeed[i] = rand() & 0xFF;
              if (secoc_sec_response(diagSeed, SEC_SEED_LEN,
                                     expectedKey, SEC_SEED_LEN) != 0) {
                sendFrameError(UDS_SID_SECURITY_ACCESS, UDS_ERROR_SECURITY_DENIED);
                return;
              }
              frameFeedback[0] = 2 + SEC_SEED_LEN;
              frameFeedback[1] = UDS_SID_SECURITY_ACCESS + 0x40;
              frameFeedback[2] = subf;
              for (i = 0; i < SEC_SEED_LEN; i++)
                frameFeedback[3 + i] = diagSeed[i];
              seedGenerated = 1;
              sendFrameFeedback(frameFeedback, 3 + SEC_SEED_LEN);
              break;
            }
            case 0x02: {
              if (frame->data[0] != (2 + SEC_SEED_LEN)) {
                sendFrameError(UDS_SID_SECURITY_ACCESS, UDS_ERROR_INCORRECT_LENGTH);
                return;
              }
              if (seedGenerated == 0) {
              ...
              // BLOCK 2A (check): constant-time compare of the submitted key
              // against expectedKey. "Constant time" means the comparison takes
              // the same time whether the key is almost-right or completely
              // wrong, so an attacker learns nothing by measuring timing.
              int i, diff = 0;
              for (i = 0; i < SEC_SEED_LEN; i++)
                diff |= frame->data[3 + i] ^ expectedKey[i];

              if (diff == 0) {
                diagSession = 0x02;
                seedGenerated = 0;
                secretSessionFound = 1;
                secFailCount = 0;          // reset the lockout counter
                frameFeedback[0] = 0x02;
                frameFeedback[1] = UDS_SID_SECURITY_ACCESS + 0x40;
                frameFeedback[2] = subf;
                ...
                // BLOCK 2B (penalty): wrong key. Throw away the seed (so the
                // attacker must request a new one), count the failure, and arm
                // the time penalty read by the gate at the top of this case:
                //   - normally a short forced wait (invalidKey, 0x35)
                //   - after SEC_MAX_ATTEMPTS in a row, a long lockout
                //     (exceededNumberOfAttempts, 0x36)
                seedGenerated = 0;
                secFailCount++;
                if (secFailCount >= SEC_MAX_ATTEMPTS) {
                  secFailCount = 0;
                  secLockUntil = currentTime + SEC_LOCK_MS;
                  sendFrameError(UDS_SID_SECURITY_ACCESS, UDS_ERROR_EXCEEDED_ATTEMPTS);
                } else {
                  secLockUntil = currentTime + SEC_DELAY_MS;
    ...
    if ((isoTpFirstFlowTime + ISOTP_TIMEOUT) >= currentTime)
      ...
      isoTpRemainingBytes = 0;  // flow-control timed out: abort the transfer
  }
}
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "rdh?")) != -1) {
    switch(opt) {
	case 'r':
		randomize = 1;
		break;
  ...
  if (randomize) {
  	...
  	if (fdseed) {
  		...
  		perror("fopen /tmp/icsim_seed.txt");
  	}
  }
  ...
  while(running) {
    while( SDL_PollEvent(&event) != 0 ) {
      ...
      pristine = 1;
      /* =====================================================================
       * SECURITY BLOCK 1 (RECEIVER SIDE) - SecOC "verify-or-drop" gate
       * =====================================================================
       * Plain-English summary:
       *   This is the single most important line of defence. Every incoming
       *   signal frame must first pass through this gate before it is allowed
       *   to change anything on the dashboard.
       *
       *   For a protected message we re-check the signature that the real
       *   sender added (SECURITY BLOCK 1, SENDER SIDE, in controls.c):
       *       - secoc_verify() recomputes the cryptographic tag with the
       *         secret key and confirms it matches  -> rejects FORGED frames
       *       - it also confirms the counter is newer than the last one we
       *         accepted                             -> rejects REPLAYED frames
       *
       *   If either check fails the frame is simply thrown away (we only log
       *   it) and the dashboard never reacts. That is exactly why an attacker
       *   on the bus can no longer fake speed, doors, turn signals, lights or
       *   the shared control channel.
       *
       *   Note: messages that are NOT in the protected list (e.g. diagnostics
       *   on 0x7E0) are out of scope here and pass straight through; they are
       *   instead defended by SECURITY BLOCK 2 inside analyseDiagRequest().
       * --------------------------------------------------------------------- */
      if (secoc_is_protected(frame.can_id) && !secoc_verify(&frame)) {
        fprintf(stderr,
                "[SecOC] dropped unauthenticated/replayed frame 0x%03X\n",
                frame.can_id);
      } else {
