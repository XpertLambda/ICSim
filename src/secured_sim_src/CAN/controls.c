 ...
 *
 * (c) Open Garages - Craig Smith <craig@theialabs.com>   [original ICSim]
 * Forked version (phil-eqtech, Barbhack CH-Workshop):
 ...
 *
 * Secured build  Copyright (c) 2026 Xpert <ahmad.pc.saad@gmail.com> - IMT Atlantique:
 *  - every transmitted signal frame is signed via secoc_sign() (SecOC sender)
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
/* ===========================================================================
 * SECURITY BLOCK 1 (SENDER SIDE) - SecOC message signing
 * ===========================================================================
 * Plain-English summary:
 *   On a normal car network (CAN bus) any device can shout any message and
 *   nobody can tell who really sent it. That is how an attacker fakes the
 *   speed, unlocks the doors, or blinks the turn signals.
 *
 *   To stop that, this control panel (the legitimate sender) stamps every
 *   signal message it broadcasts with a tamper-proof "signature" before it
 *   goes out on the wire. The signature is produced by secoc_sign() and is
 *   made of two parts:
 *       1. a counter that goes up by one every time  -> blocks REPLAY
 *          (an attacker cannot record a message and play it back later)
 *       2. a cryptographic tag (a MAC) computed with a secret key that only
 *          the real ECUs know                        -> blocks SPOOFING
 *          (an attacker cannot forge the tag without the key)
 *
 *   Every send function below therefore ends with:
 *       sendPkt(secoc_sign(&cf));
 *   i.e. "sign the frame, then put it on the bus". The matching check on the
 *   receiving side is SECURITY BLOCK 1 (RECEIVER SIDE) in icsim.c.
 *
 *   See secoc.c / secoc.h for the crypto, and SECURITY_LAYER.md for the full
 *   write-up.
 * ------------------------------------------------------------------------- */
...
/* Sends a raw diagnostic frame (ID 0x7E0) over the bound socket.
 * The control panel acts as the tester here; diagnostics are intentionally
 * NOT SecOC-protected (out of scope), so these go out unsigned.  Sending via
 * the socket - rather than shelling out to `cansend` on a hardcoded vcan0 -
 * means the diag buttons work on whatever interface was passed on the CLI. */
void sendDiag(const unsigned char *data, int len) {
  struct canfd_frame d;
  memset(&d, 0, sizeof(d));
  d.can_id = 0x7E0;
  d.len = len;
  if (len > CAN_MAX_DLEN) len = CAN_MAX_DLEN;
  memcpy(d.data, data, len);
  if (write(s, &d, CAN_MTU) != CAN_MTU) perror("write diag");
}
...
int main(int argc, char *argv[]) {
  ...
  while(running) {
          ...
          switch(event.key.keysym.sym) {
            ...
            case SDLK_d:
              if (diagButtonPressed == 0) {
                ...
                if (diagOn == 0) {
                  diagActive = 0;
                  sendDiag((const unsigned char[]){0x02,0x10,0x01,0,0,0,0,0}, 8);
                } else {
                  sendDiag((const unsigned char[]){0x02,0x10,0x02,0,0,0,0,0}, 8);
                }
                diagButtonPressed = 1;
              }
              break;
            case SDLK_x:
              if (diagOn == 1 && routineButtonPressed == 0) {
                diagActive ^= 1;
                if (diagActive == 1)
                  sendDiag((const unsigned char[]){0x04,0x31,0x41,0x10,0x01,0,0,0}, 8);
                else
                  sendDiag((const unsigned char[]){0x04,0x31,0x41,0x10,0x00,0,0,0}, 8);
    ...
    if (diagOn == 1 && (lastDiagTesterPresent + 1000) < currentTime) {
      ...
      sendDiag((const unsigned char[]){0x02,0x3E,0,0,0,0,0,0}, 8);  // TesterPresent
    }
