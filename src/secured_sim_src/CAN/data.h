 ...
 *
 * data.h - CAN signal IDs, UDS service/NRC constants, and tuning parameters.
 *
 * Original: Barbhack CH-Workshop (phil-eqtech)
 * Security-layer additions (Layer 2 UDS NRCs and anti-brute-force tuning):
...
#define UDS_ERROR_BUSY_REPEAT             0x21 // busyRepeatRequest (rate limit)
#define UDS_ERROR_SEQUENCE_ERROR          0x24
#define UDS_ERROR_SECURITY_DENIED         0x33
#define UDS_ERROR_INVALID_KEY             0x35
#define UDS_ERROR_EXCEEDED_ATTEMPTS       0x36 // exceededNumberOfAttempts (lockout)
#define UDS_ERROR_TIME_DELAY              0x37 // requiredTimeDelayNotExpired
#define UDS_ERROR_REQUEST_OUT_RANGE       0x31

// --- SECURITY BLOCK 2 tuning knobs (secured sim only) ----------------------
// These numbers control how aggressive the diagnostic hardening is. They are
// gathered here so the behaviour can be tuned without touching the logic in
// icsim.c. See SECURITY_LAYER.md for the full explanation of each block.
//
// BLOCK 2A/2B - SecurityAccess (service 0x27) strong key + anti-brute-force
#define SEC_MAX_ATTEMPTS                  3      // wrong keys in a row before a long lockout
#define SEC_DELAY_MS                      1000   // forced wait (ms) after EACH wrong key
#define SEC_LOCK_MS                       10000  // lockout (ms) once SEC_MAX_ATTEMPTS is hit
#define SEC_SEED_LEN                      4      // length (bytes) of the random seed / expected key
// BLOCK 2D - RoutineControl (service 0x31) rate limit; defeats 0x31 scanners
#define ROUTINE_MIN_INTERVAL_MS           250    // minimum gap (ms) allowed between routines
